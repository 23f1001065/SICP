...
def sponsor_register():
    if request.method == "POST":
        ...
        try:
            ...
            if sponsor:
                return render_template('message.html',id='SP')
            else:
                newSponsor = Sponsor()
                ...
                db.session.commit()
                return render_template('message.html', id='SPRSUCC',sponsor_id=newSponsor.sponsor_id,password=password)
    ...
    if request.method =="POST":
        ...
        try:
            ...
            if sponsor and password == sponsor.password:
                ...
                return 'Invalid Credentials. Sponsor with that ID or password not Found', 404
...
def about_adrequest(flag,ad_id):
    ...
    image = session['image']
    try:
        adrequest = Adrequest.query.filter_by(adId = ad_id).first()
    except Exception as e:
         return render_template('message.html', id = 'DBERROR', error = e),503 
    if flag == 'v':
...
def about_camp(flag,camp_id):
    ...
    if flag == 'v':
        ...
        if campaign and request.method == 'POST':
            try:
                campaign.campaign_id = generate_random_id("CAMP")
                ...
                campaign.created_date = datetime.now().strftime('%Y-%m-%d %H:%M')
            except Exception as e:
                return render_template('message.html', id = 'DBERROR', error = e),503
            else:
                db.session.commit()
